...
3. **Replace perl with python3** while you're in there. Perl is currently used for the existing measurement helpers; python3 is the preferred modernisation target (available on macOS by default since 12.3, on every Linux distro, on the GitHub Actions runner image). Python's `len(str)` after `re.sub('\x1b\[[0-9;]*m', '', s)` is the equivalent of the current perl one-liner. Adds ~20ms startup vs perl's ~10ms, still well under the perception threshold.
