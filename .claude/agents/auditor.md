...
PUBLIC GitHub repo (github.com/vtmocanu/cc-statusline). Security-scan slot:
`none (gap)` — no gitleaks/semgrep/trivy config and no scanner step in CI, so
there is nothing for you to run; say that plainly rather than implying a clean
scan. Secret scanning is GitHub-native (push protection on). Other gates:
`task ci`.
...
- `hooks/session-topic-capture.sh` and `claude-usage-fetch.sh` handle the
  ...
  `cc-statusline/X.Y.Z` and does NOT impersonate the official client (a
  `claude-code/...` UA was removed in v2.0.1 for exactly this reason).
...
- Untrusted ENVIRONMENT inputs: `COLUMNS`/`LINES` feed arithmetic that narrows
  ...
  unguarded arithmetic error means no statusline at all.
- Cache and lock files live under a per-user mode-700 state dir (`_state_dir`),
  not a predictable `/tmp` path; flag any regression to a world-writable
  location or a guessable filename.
- CI baseline to defend: top-level `permissions: contents: read`,
  `persist-credentials: false` on checkout, every action SHA-pinned with a
  trailing version comment, no `pull_request_target`. Flag any regression.
